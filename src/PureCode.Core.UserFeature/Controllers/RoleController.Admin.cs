...
namespace PureCode.Core.Controllers
{
  public partial class RoleController : ControllerBase
  {
    ...
    //public AjaxResponse<IEnumerable<PermissionModel>> Permissions()
    //{
    ...
    public async Task<PagedAjaxResponse<RoleModel>> IndexAsync(int page = 1, int size = 1000)
    {
      return new PagedAjaxResponse<RoleModel>
      {
    ...
    public async Task<AjaxResponse<RoleModel>> GetRole(string name)
    {
    ...
    public async Task<AjaxResponse<RoleModel>> GetRoleWithoutClaims(string name)
    {
      return new AjaxResponse<RoleModel>
      {
        Code = 0,
    ...
    public async Task<AjaxResponse> Add([FromBody] RoleModel rwuac)
    {
      ...
      return new AjaxResponse { Message = "保存成功" };
    }

    [HttpPost("/api/admin/roles/{roleName}/users", Name = "管理员 - 分配角色用户")]
    public async Task<AjaxResponse> UpdateRoleUsers(string roleName, [FromBody] List<UserModel> users)
    {
      await userManager.DeleteUserRoleAsync(roleName);
      if (users == null || users.Count == 0) return new AjaxResponse { Message = "保存成功，已清空角色用户" };
      await userManager.AddUserRoleAsync(roleName, users!);
      return new AjaxResponse { Message = "保存成功" };
    }

    [HttpPost("/api/admin/roles/{name}/update", Name = "管理员 - 修改角色")]
    public async Task<AjaxResponse> Update(string name, [FromBody] RoleModel roleWithUsersAndClaims)
    {
    ...
    public async Task<AjaxResponse<bool>> IsExistRole(string name)
    {
    ...
    public async Task<AjaxResponse> Delete(string roleName)
    {
      await roleManager.DeleteRoleClaimAsync(roleName);
      await userManager.DeleteUserRoleAsync(roleName);
      await roleManager.DeleteRoleAsync(roleName);
      return new AjaxResponse { Message = "操作成功" };
    }
  }
}