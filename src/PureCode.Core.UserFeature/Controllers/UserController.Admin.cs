...
public partial class UserController
{
  ...
  public async Task<PagedAjaxResponse<AdminUserModel>> AdminListUsersAsync(string? query = null, int page = 0,
    int size = 10)
  {
    ...
    return new PagedAjaxResponse<AdminUserModel>
    {
      Total = total,
      Page = page,
      Data = users
  ...
  public async Task<PagedAjaxResponse<AdminUserModel>> AdminListDeletedUsersAsync(string? query = null, int page = 0,
    int size = 20)
  {
    var dict = new Dictionary<string, object?>
    {
  ...
  public async Task<PagedAjaxResponse<AdminUserModel>> AdminListInvisibleUsersAsync(string? query = null, int page = 0,
    int size = 20)
  {
    var dict = new Dictionary<string, object?>
    {
  ...
  public async Task<AjaxResponse<UserModel>> AdminCreateUserAsync([FromBody] UserModel user)
  {
    if (string.IsNullOrEmpty(user.UserName))
    {
      return new AjaxResponse<UserModel>
      {
        Code = 403,
        Message = "用户名为空，无法添加"
      };
  ...
  public async Task<AjaxResponse<UserModel>> AdminUpdateAsync(string userName, [FromBody] UserModel user)
  {
  ...
  public async Task<AjaxResponse<bool>> AdminDeleteUserAsync(string userName)
  {
    var data = await userManager.SetDeletedAsync(userName, true);
    return new AjaxResponse<bool> { Data = data };
  }

  [HttpPost("/api/admin/users/{userName}/active", Name = "管理员 - 激活用户")]
  public async Task<AjaxResponse<bool>> AdminActiveUserAsync(string userName)
  {
  ...
  public async Task<AjaxResponse<bool>> ResetPassword(string userName)
  {
  ...
  public async Task<AjaxResponse<bool>> AdminIsExistUserAsync(string userName)
  {
  ...
  public AjaxResponse AdminUploadAvatar(IFormFile avatar)
  {
    var data = profileManager.UploadImage(avatar);
    return new AjaxResponse { Data = data };
  }

  [HttpGet("/api/admin/users/{userName}", Name = "管理员 - 用户基础信息")]
  public async Task<AjaxResponse<UserModel>> GetBaseUserAsync(string userName)
  {
    var user = await userManager.FindByNameAsync(userName);
    var data = await userManager.GetBriefUserAsync(user!.Id, user.UserName!,
      new string[] { "public", "surname", "givenName" });
    return new AjaxResponse<UserModel> { Data = data };
  }
}