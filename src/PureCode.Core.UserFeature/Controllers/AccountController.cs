  ...
  : ControllerBase
{
  ...
  public async Task<AjaxResponse<CaptchaModel>> GenerateCaptcha()
  {
    if (!captchaManager.Enabled)
    {
      return new AjaxResponse<CaptchaModel>();
    }
  ...
  public async Task<AjaxResponse<AuthenticationTokens>> LoginAsync([FromBody] LoginUserModel user)
  {
    if (captchaManager.Enabled)
    {
      ...
      {
        return new AjaxResponse<AuthenticationTokens>
        {
          Code = 400,
  ...
  public async Task<AjaxResponse<UserModel>> GetProfileAsync()
  {
  ...
  [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme,
    Policy = PermissionClaimNames.ApiPermission)]
  public async Task<AjaxResponse<UserModel>> UpdateProfileAsync([FromBody] Dictionary<string, JsonElement?> profiles)
  {
  ...
  [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme,
    Policy = PermissionClaimNames.ApiPermission)]
  public AjaxResponse UploadAvatar(IFormFile avatar)
  {
  ...
  [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme,
    Policy = PermissionClaimNames.ApiPermission)]
  public async Task<AjaxResponse> GetPermissionsAsync()
  {
  ...
  [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme,
    Policy = PermissionClaimNames.ApiPermission)]
  public async Task<AjaxResponse> UpdatePasswordAsync([FromBody] ChangePasswordModel passwords)
  {
    var user = await userManager.GetUserAsync(HttpContext.User);
    if (user != null)
    {
