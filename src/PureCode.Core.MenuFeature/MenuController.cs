...
public partial class MenuController(MenuManager menuManager, UserManager userManager,RoleManager roleManager) : ControllerBase
{
  ...
  public async Task<AjaxResponse<ICollection<MenuItemModel>?>> GetMainMenu()
  {
    ...
    if (user != null)
    {
      ...
      return new AjaxResponse<ICollection<MenuItemModel>?> { Data = userMenu };
    }
    else
    {
      return new AjaxResponse<ICollection<MenuItemModel>?> { Code = 404, Message = "未找到用户信息 [导航菜单]" };
    }
  }
}